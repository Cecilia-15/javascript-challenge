...
const paddockManagers = [
  ...
  */
  ...
  export const listPaddockManagersByName = () => {
    const listByName = paddockManagers.sort(function(a,b){
      if(a.name < b.name) {
        return -1
    ...
    return listByName.map(({ taxNumber }) => taxNumber)
  }
  ...
  export const sortPaddockTypeByTotalArea = () => {
    const paddockAux = paddockType.map(paddockmap => {
      let sum = 0
      paddocks.forEach(padd => {
        if(paddockmap.id === padd.paddockTypeId) {
          sum = sum + padd.area
        }
      })
      paddockmap.area = sum
    ...
    return paddockAux.sort((a, b) => b.area - a.area).map((paddockmap) => paddockmap.name)
  }
  ...
  export const sortFarmManagerByAdminArea = () => {
    const total = {}
    paddocks.forEach(padd => {
      const {paddockManagerId, area} = padd
